...
in {
  ...
  options.godtamnix.theme.catppuccin = {
    ...
    lib.mkMerge [
      (lib.optionalAttrs (inputs ? catppuccin && inputs.catppuccin ? homeModules) {
        catppuccin = {
          ...
          cava = enabled;
          delta = enabled;
          # firefox = mkIf config.godtamnix.programs.graphical.browsers.firefox.enable {
          #   profiles.${config.godtamnix.user.name} = {
          #     enable = true;
          #     force = true;
          #   };
          # };
          ...
          thunderbird = {
            enable = true;
            profile = config.godtamnix.user.name;
          };
          tmux = enabled;
...

      {
        home = {
          file = mkMerge [
            ...
            #   in
            #   mkIf config.godtamnix.programs.terminal.emulators.warp.enable {
          ...
          sessionVariables = mkIf pkgs.stdenv.hostPlatform.isLinux {
            CURSOR_THEME = config.godtamnix.theme.gtk.cursor.name;
          };
        };

        programs = {
          # Additional program settings that don't follow the common pattern
          satty.settings = mkIf config.godtamnix.programs.graphical.addons.satty.enable {
            color-palette = {
              palette = [
